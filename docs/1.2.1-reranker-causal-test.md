<!-- Updated: 2026-03-04 (Phase 3: v2.1 golden + alternative approaches) -->
...
## Phase 2: Re-evaluation After Grid Search (2026-03-04)

### Why Revisit the No-Go Decision

The grid search experiment (`notebooks/threading_gridsearch.ipynb`, `docs/4-threading-gridsearch-report.md`) revealed a problem that threshold tuning alone cannot solve:

**Fragmentation = 47.8%** — nearly half of golden threads have their articles split across multiple system threads, even with optimal parameters.

This directly contradicts the original No-Go conclusion that "threshold tuning achieves most of the quality improvement." Threshold tuning optimized contamination and causal recall, but fragmentation — the problem cross-encoders are designed to solve — remains high.

### The Original No-Go Was Answering the Wrong Question

| Original question | New question |
|------------------|-------------|
| "Can cross-encoder **replace** cosine?" | "Can cross-encoder **complement** cosine?" |
| Tested: all 2,427 pairs through cross-encoder | Needed: only ambiguous pairs (cosine 0.40-0.75) |
| Result: 822s too slow for full pipeline | Actual need: ~50-100 pairs/day → ~17s |

### Contradicting Evidence

The grid search also showed:
- **Reranker experiment (Feb)**: Raise threshold 0.62 → 0.73 (reduce contamination)
- **Grid search (Mar)**: Lower threshold 0.73 → 0.65 (reduce fragmentation)

These point in opposite directions because they optimize different metrics. The real answer is: **use both** — cosine for fast filtering, cross-encoder for the ambiguous middle zone.

### Updated Verdict: Conditional Go for 2-Stage Pipeline

Cross-encoder is **Go** as a second stage, **not** as a replacement:

```
Article arrives
    │
    ▼
[Stage 1: Cosine similarity] — fast, free
    │
    ├─ sim > 0.75 → match (high confidence)
    ├─ sim < 0.40 → no match (clearly unrelated)
    │
    ▼
[Stage 2: Cross-encoder] — slow, but only for ambiguous zone
    │
    ├─ 0.40 ≤ sim ≤ 0.75 → Qwen3 scores the pair
    │
    ▼
Final assignment
```

### Phase 4.6 Experiment Plan

**Notebook**: Append to `notebooks/threading_gridsearch.ipynb` (Cells 15-19)
**Goal**: Measure fragmentation reduction from 2-stage matching

**Design**:
1. Take best grid search params (`base_threshold=0.65`)
2. Run simulation, identify where fragmentation occurs
3. For fragmented golden threads, extract the "missed" article-thread pairs
4. Score these pairs with Qwen3-Reranker-0.6B
5. Re-run evaluation with cross-encoder rescues included
6. Measure: fragmentation 47.8% → target < 30%

**Go/No-Go for integration**:
| Criterion | Threshold |
|-----------|-----------|
| Fragmentation reduction | > 10pp absolute (47.8% → < 38%) |
| Contamination increase | < 5pp (don't break what works) |
| Runtime per daily batch | < 60s for ~50-100 ambiguous pairs |
| Causal recall maintained | ≥ 0.90 |
...
| File | Purpose |
|------|---------|
| `notebooks/reranker_causal_test.ipynb` | Phase 1 experiment (17 cells) |
| `notebooks/reranker_causal_labels.pkl` | Saved labels + pairs + model scores (2.2MB) |
| `notebooks/threading_gridsearch_v2.ipynb` | Phase 2-3 experiments (Cells 1-20) |
| `notebooks/golden_dataset_v2.1.json` | Golden dataset v2.1 (170 threads, 801 singletons) |
| `scripts/7_embed_and_thread.py` | Current threading pipeline |
| `docs/1.2-news-threading.md` | Threading system design |

---

## Phase 3: Golden v2.1 + Alternative Approaches (2026-03-04)

### Golden Dataset v2.1

v1 golden (134 threads) had garbage-bin threads (30-60 unrelated articles mixed together). Rebuilt using split-then-merge:
1. HDBSCAN split large threads into sub-clusters
2. Gemini validated each sub-cluster (strict: same specific event only)
3. Gemini merge pass recombined over-split threads
4. Pairwise spot-check: **88% agreement** (sufficient quality)

**v2.1 stats**: 170 threads, 801 singletons, 2102 articles.

### Grid Search on v2.1

234 combos (focused on 3 sensitive params). Best v2.1 params:
...
`centroid_decay` and `entity_weight` had negligible impact — fixed at 0.08 and 0.04.

### CE Rescue — No-Go (Confirmed)

Tested CE rescue two ways:
1. **Oracle** (golden labels identify candidates): composite 0.7539 → 0.9269. But uses answer key — not realistic.
2. **Production-realistic** (heuristic candidate selection): composite 0.7539 → 0.7464. **Worse than baseline.**

CE cannot identify which articles are misplaced without ground truth. Rescue approach abandoned.

### Alternative Approaches Tested

**Notebook**: `threading_gridsearch_v2.ipynb` Cells 17-20.

**Sources**:
- [Real-time News Story Identification](https://arxiv.org/abs/2508.08272) — Škvorc et al. 2025
- [Noise-Robust De-Duplication at Scale](https://arxiv.org/abs/2210.04261) — Dell et al.
- [Event-Driven News Stream Clustering (EACL 2021)](https://aclanthology.org/2021.eacl-main.198/)
- [Hierarchical News Clustering via Matryoshka Embeddings](https://arxiv.org/html/2506.00277v1)
...
| Better embeddings (BGE/GTE) | ~0.61 greedy | 0.55+ | ~0.12 | No-Go |

### Greedy + CE Merge — Go (Recommended)

After greedy cosine threading, find similar thread pairs (centroid cosine >= 0.60) and use CE (GTE-ModernBERT) to confirm merge (CE >= 0.6). Uses union-find to handle transitive merges.

**Why this works**: CE is excellent at confirming "are these two threads the same story?" — a much easier question than "is this article in the wrong thread?"

**Why not Louvain**: Higher composite but contamination 0.22 (vs 0.009). Not streaming-compatible. Fragmentation advantage (0.20 vs 0.36) will be addressed by Phase 6 parent threading.

**Why not better embeddings**: OpenAI text-embedding-3-small outperforms BGE-large and GTE-ModernBERT for greedy matching. Local models only match OpenAI when using Louvain.

### Final Recommended Config

```
Stage 1 — Greedy cosine (unchanged pipeline, new params):
  THREAD_BASE_THRESHOLD = 0.60
  AUTHOR_BOOST_THRESHOLD = 0.55
  THREAD_TIME_PENALTY = 0.005

Stage 2 — CE merge pass (new, runs after threading):
  Model: GTE-ModernBERT (Alibaba-NLP/gte-reranker-modernbert-base, 149M)
  Centroid threshold: 0.60 (candidate pairs)
  CE threshold: 0.60 (merge decision)
  Runtime: ~150s for ~12K pairs (one-time daily)
```
