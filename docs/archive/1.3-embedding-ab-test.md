...
A/B 테스트 후 `BAAI/bge-base-en-v1.5` (110M params, 768d)로 교체 완료 — Gemini 호출 62.8% 절감.
이 문서는 교체 결정의 근거가 된 실제 DB 데이터(1,951 labeled pairs) 측정 결과.
...
flowchart TD
    ...
    C --> D["Pre-crawl Ranking<br/>(4_embedding_rank.py)<br/>MiniLM scores → top_k=5"]
    D --> E[Crawl top 5 URLs]
    E --> F["Post-crawl Scoring<br/>(6_crawl_ranked.py)<br/>MiniLM: WSJ text vs content[:800]"]
...
# Matches 6_crawl_ranked.py line 449 accept logic
```
...
**Result**: No prefix is slightly better. Current `7_embed_and_thread.py` behavior (no prefix) is correct.
...
- Already installed (`7_embed_and_thread.py` uses it)
...
For thread creation (`7_embed_and_thread.py`), the workflow is:
...
```python
# In 6_crawl_ranked.py, after computing embedding score with bge-base:
...
## Cost Analysis: 3-Zone Gate Under Current 2-Step Architecture (2026-03-05)

> **Conclusion: Not worth implementing for cost savings.** The 62.8% figure was estimated when there was a single Gemini call. The pipeline now uses a 2-step architecture where Step 1 (gate) is already very cheap.

### Current Pipeline (2-Step)

| Step | Model | Role | Price (per 1M tokens) |
|------|-------|------|----------------------|
| Step 1 (Gate) | Gemini 2.5 Flash Lite | relevance + is_same_event | in: $0.10, out: $0.40 |
| Step 2 (Analysis) | Gemini 2.5 Flash | headline, summary, keywords, entities | in: $0.30, out: $2.50 |

### Per-Article Cost (~2,000 content tokens)

| | Input tokens | Output tokens | Cost |
|--|-------------|--------------|------|
| Step 1 | ~2,200 | ~80 | **$0.00025** |
| Step 2 | ~2,300 | ~400 | **$0.00169** |

Step 2 is ~7x more expensive than Step 1.

### 3-Zone Gate Impact (per 100 articles)

**Current (no gate):**
- Step 1: 100 calls → $0.025
- Step 2: ~65 calls (pass rate) → $0.110
- **Total: ~$0.135**

**With 3-Zone Gate:**

| Zone | Articles | Step 1 | Step 2 |
|------|----------|--------|--------|
| Accept (≥0.73) | 43 | Skip | Run (still need headline/summary) |
| Reject (≤0.59) | 20 | Skip | Skip |
| Uncertain | 37 | Run | ~24 pass |

- Step 1: 37 calls → $0.009
- Step 2: 67 calls → $0.113
- **Total: ~$0.122**

### Monthly Estimate (~30 articles/day)

| | Monthly cost | Savings |
|--|-------------|---------|
| Current | ~$1.22 | — |
| With 3-Zone Gate | ~$1.10 | **~$0.12/mo (~10%)** |

### Why the Original 62.8% Estimate No Longer Applies

The original analysis assumed a single Gemini call for both gating and analysis. The pipeline was later split into:
1. **Step 1 (Flash Lite)**: Cheap gate — $0.00025/article
2. **Step 2 (Flash)**: Expensive analysis — $0.00169/article

The 3-Zone Gate mainly skips Step 1 (already cheap). Step 2 is still required for accepted articles because it generates headline, summary, key_takeaway, keywords, entities, and importance — none of which the embedding score can replace. The only real savings come from the reject zone (20% of articles skip both steps).
...
## Appendix: Search Pipeline A/B Test (2026-02-21)

Separate from the embedding model A/B test above. This tested the full search pipeline changes (LLM queries, tighter date window, broader site exclusions) vs the old main branch.
