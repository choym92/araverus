...
### Task 6: Fix today_only Filter

**Status**: [x] COMPLETED

**Problem**: `load_wsj_jsonl()` had `today_only=True` default, which skipped older WSJ items entirely. This meant only items published today were processed.

**Clarification**: The intended date filtering was for Google News results, not input items:
- ❌ Wrong: Skip WSJ items not published today
- ✅ Correct: Use WSJ pubDate to filter Google News search results

**Fix**: Changed `today_only: bool = True` → `today_only: bool = False` in `load_wsj_jsonl()`.

**Note**: Google News date filtering already correctly uses `add_date_filter()` with WSJ pubDate (lines 639-648).

---

### Task 7: Add Searched Flag to Prevent Re-searching

**Status**: [x] COMPLETED

**Problem**: Items without successful crawls were re-searched on Google News every run, wasting API calls since news becomes stale quickly.

**Solution**: Two-flag system:
- `searched` = Google News search was attempted
- `processed` = Successful crawl completed

**Database changes**:
```sql
ALTER TABLE wsj_items
ADD COLUMN IF NOT EXISTS searched boolean NOT NULL DEFAULT false,
ADD COLUMN IF NOT EXISTS searched_at timestamptz;

CREATE INDEX IF NOT EXISTS idx_wsj_items_unsearched
ON wsj_items(searched, created_at) WHERE NOT searched;
```

**Code changes**:
1. `wsj_ingest.py`:
   - Changed `get_unprocessed_items()` → `get_unsearched_items()` (query `searched=false`)
   - Added `mark_items_searched()` function
   - Added `--mark-searched FILE` CLI command
2. `.github/workflows/finance-pipeline.yml`:
   - Added step 4: Mark items as searched after Google News search

**Updated flow**:
```
Export (searched=false) → Google News → Mark searched=true → Rank → Crawl → Mark processed=true
```
