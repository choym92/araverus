...
**Completed:**
1. Fixed documentation inconsistencies in `docs/1-news-backend.md`
...
- `docs/1-news-backend.md` — comprehensive updates (lines, models, env vars, Stage 1 details)
...
# Frontend Building Session — 2026-02-18
...
2. **Rewrote `ArticleCard.tsx`** (major changes)
   ...
   - Thread indicator at card bottom: `◀ N/M ▶ thread-name` with disabled boundaries
...
3. **Updated `page.tsx`** (layout restructuring)
   ...
   - **36h → 24h cutoff** with fallback: if < 6 recent articles, backfill with older ones
...
5. **Verification**
   - `npm run lint` — no new errors (existing BriefingPlayer warnings only)
   - `npm run build` — success, `/news` compiles to 6.55kB
...
- Solution: **24h cutoff with fallback logic** — if < 6 articles, fetch without time filter
...
| Carousel starts at latest | User naturally scrolls backward (◀) to see older articles in thread |
| Remove `compact` variant | Right column uses same `standard` cards; simplifies styling + consistency |

## Files Changed

**Major rewrites:**
- `src/app/news/_components/ArticleCard.tsx` — full client-side carousel implementation
- `src/app/news/page.tsx` — 3-column layout as primary, data fetching restructure

**Modifications:**
- `src/lib/news-service.ts` — added `since` parameter to `getNewsItems()`

**Deleted:**
- `src/app/news/_components/ThreadSection.tsx` — removed entirely
...
- [ ] **Confirm pipeline stages** — check Mac Mini logs to see which stages (crawl, LLM, thread, etc.) ran
- [ ] **Decide on crawl-missing articles** — show them with basic RSS data? or require full crawl?
- [ ] **Add Google News RSS fallback** (optional, user mentioned) — if crawl fails, show uncrawlable items from RSS to user
- [ ] **Test carousel interactions** — verify slide direction, boundary states, thread title truncation
- [ ] **Verify thread timeline ordering** — ensure timeline is sorted ascending by `published_at`
...
- Thread timeline always sorted ascending by `published_at` (oldest → newest)

### Data Flow
```
page.tsx:
  → fetch today's items (24h)
  → fetch all items (fallback if < 6 recent)
  → fetch thread timelines in parallel
  → build threadTimelines Map<threadId, NewsItem[]>
  → render 3-column layout with carousel props
```
...
- Indicator format: `◀ 19/19 ▶ Thread Title 📰`

### Known Gotchas
- **Thread timeline is oldest → newest** (asc by published_at); carousel starts at `length-1` (latest)
- **Carousel state is local only** — refreshing page resets position to latest
- **No scroll restoration** — moving between threads doesn't remember carousel position in each
- **Preview images:** Some articles missing `top_image`; cards render without image section

### Environment
- Turbopack dev server on localhost:3000
- `.next` cache can corrupt from concurrent writes — `rm -rf .next/` + restart fixes
...
```
6f6ccc9 fix: backend Stage 1 (wsj_ingest.py) audit & cleanup
b2f175a feat: thread titles, heat ranking, collapse + pipeline/detail fixes
d96be12 docs: consolidate news UX plans into single file
ec23928 docs: add frontend Phase 6-8 plan (thread title, collapse, heat ranking)
648b8f3 chore: sync branch with all pending changes
2388c45 feat: threading v3.3 — anti-gravity overhaul, max 17 members
```
